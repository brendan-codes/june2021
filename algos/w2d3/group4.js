...
function isPalindrome(queue) {
    ...
    while(queue){
        ...
        let whileQueue = queue.dequeue();
        checkOriginalQueue.push(whileQueue.data);
        tempQueue.enqueue(whileQueue);
    }

    while(tempQueue){
        if(tempQueue.isEmpty()) break;
        let whileQueue = tempQueue.dequeue();
        checktempQueue.push(whileQueue.data);
        queue.enqueue(whileQueue);
    }

    checktempQueue.reverse();
    if (checkOriginalQueue.toString() == checktempQueue.toString()) {
        returnBoolean = true;
    }
